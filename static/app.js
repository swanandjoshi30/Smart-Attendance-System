...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Photo upload bindings for enrollment
    const btnUploadTrigger = document.getElementById("btn-upload-trigger");
    const enrollFileInput = document.getElementById("enroll-file");
    if (btnUploadTrigger && enrollFileInput) {
        btnUploadTrigger.addEventListener("click", () => enrollFileInput.click());
        enrollFileInput.addEventListener("change", (e) => handlePhotoUpload(e, "enroll"));
    }

    // Photo upload bindings for editing student
    const btnEditUploadTrigger = document.getElementById("btn-edit-upload-trigger");
    const editFileInput = document.getElementById("edit-file");
    if (btnEditUploadTrigger && editFileInput) {
        btnEditUploadTrigger.addEventListener("click", () => editFileInput.click());
        editFileInput.addEventListener("change", (e) => handlePhotoUpload(e, "edit"));
    }

    // Bulk registration templates
    const btnDownloadStudentTemplate = document.getElementById("btn-download-student-template");
    if (btnDownloadStudentTemplate) {
        btnDownloadStudentTemplate.addEventListener("click", downloadStudentCSVTemplate);
    }
    const btnDownloadSubjectTemplate = document.getElementById("btn-download-subject-template");
    if (btnDownloadSubjectTemplate) {
        btnDownloadSubjectTemplate.addEventListener("click", downloadSubjectCSVTemplate);
    }

    // Bulk registration imports triggers
    const btnImportStudentsTrigger = document.getElementById("btn-import-students-trigger");
    const importStudentsFileInput = document.getElementById("import-students-file");
    if (btnImportStudentsTrigger && importStudentsFileInput) {
        btnImportStudentsTrigger.addEventListener("click", () => importStudentsFileInput.click());
        importStudentsFileInput.addEventListener("change", () => importBulkCSV(importStudentsFileInput, "students"));
    }

    const btnImportSubjectsTrigger = document.getElementById("btn-import-subjects-trigger");
    const importSubjectsFileInput = document.getElementById("import-subjects-file");
    if (btnImportSubjectsTrigger && importSubjectsFileInput) {
        btnImportSubjectsTrigger.addEventListener("click", () => importSubjectsFileInput.click());
        importSubjectsFileInput.addEventListener("change", () => importBulkCSV(importSubjectsFileInput, "subjects"));
    }

    // Camera form submit & cancel bindings
    ...
    if (btnCameraCancel) {
        btnCameraCancel.addEventListener("click", resetCameraForm);
    }
});
...
function switchTab(tabId) {
    ...
    if (tabId === "analytics-tab") {
        pageTitle.textContent = "Attendance Analytics Dashboard";
        pageSubtitle.textContent = "Visual trends, performance metrics and classroom insights";
        loadAnalytics();
    }

    if (tabId === "cameras-tab") {
        pageTitle.textContent = "Configure CCTV Camera Sources";
        pageSubtitle.textContent = "Register, configure, and monitor live IP camera nodes";
        loadCamerasDirectory();
    }
}
...
function renderLogsReportTable() {
    ...
    if (selectedMode === "cumulative") {
        ...
        if (filteredStudents.length === 0) {
            tbody.innerHTML = `<tr><td colspan="${isAdmin ? 6 : 5}" class="center">No matching student records found.</td></tr>`;
            return;
        }
                ...
                actionHtml = `
                    <td>
                        <div class="action-btn-group">
                            ${status === 'present' ? 
                                `<button onclick="overrideAttendance(${sessionId}, '${student.prn}', 'absent')" class="btn-delete-tbl" style="padding: 4px 8px; font-size: 0.75rem;"><i class="fa-solid fa-circle-xmark"></i> Absent</button>` : 
                                `<button onclick="overrideAttendance(${sessionId}, '${student.prn}', 'present')" class="btn-edit-tbl" style="padding: 4px 8px; font-size: 0.75rem;"><i class="fa-solid fa-circle-check"></i> Present</button>`
                            }
                        </div>
                    </td>
                `;
...
// ------------------ NEW V2 HANDLERS: FILE UPLOAD, BULK CSV, ANALYTICS ------------------

function handlePhotoUpload(event, mode) {
    ...
    reader.onload = function(e) {
        ...
        if (mode === "enroll") {
            ...
            // Toggle displays like capture enroll photo
            enrollVideo.classList.add("hide");
            capturedImg.classList.remove("hide");
            btnCapture.classList.add("hide");
            btnRetake.classList.remove("hide");
...
function downloadStudentCSVTemplate() {
    const csvContent = "data:text/csv;charset=utf-8,prn,class_id,roll_no,name,email\n72345678X,1,101,John Doe,john.doe@college.edu\n";
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
...
function downloadSubjectCSVTemplate() {
    const csvContent = "data:text/csv;charset=utf-8,subject_code,subject_name,class_id,semester\nCS101,Computer Programming,1,1\n";
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
...
async function importBulkCSV(fileInput, type) {
    const file = fileInput.files[0];
    if (!file) return;

    const statusDiv = document.getElementById(`import-${type}-status`);
    statusDiv.classList.remove("hide");
    statusDiv.className = "error-msg"; // Reset styling
    statusDiv.style.background = "rgba(76, 201, 240, 0.15)";
    statusDiv.style.color = "#4cc9f0";
    statusDiv.style.border = "1px solid rgba(76, 201, 240, 0.3)";
    statusDiv.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> Uploading and importing ${type}...`;

    const formData = new FormData();
    formData.append("file", file);

    try {
        const response = await fetch(`${API_URL}/api/import/${type}`, {
            method: "POST",
            body: formData
        });
        ...
        const data = await response.json();
        if (!response.ok) {
            throw new Error(data.detail || `Failed to import ${type}`);
        }

        statusDiv.style.background = "rgba(16, 185, 129, 0.15)";
        statusDiv.style.color = "#10b981";
        statusDiv.style.border = "1px solid rgba(16, 185, 129, 0.3)";
        statusDiv.innerHTML = `<i class="fa-solid fa-circle-check"></i> ${data.detail || 'Import successful!'}`;
        
        // Reload application data after a short delay
        setTimeout(() => {
            loadApplicationData();
            if (type === "students") {
                loadEnrolledStudents();
        ...
        statusDiv.innerHTML = `<i class="fa-solid fa-circle-xmark"></i> Import failed: ${e.message}`;
    } finally {
...
async function loadAnalytics() {
    try {
        const resStats = await fetch(`${API_URL}/api/analytics/class-stats`);
        const classStats = await resStats.json();
        
        const resTrends = await fetch(`${API_URL}/api/analytics/trends`);
...
function renderClassChart(data) {
    ...
    data.sort((a, b) => a.class_name.localeCompare(b.class_name));
    const labels = data.map(item => item.class_name);
    const dataset = data.map(item => item.avg_attendance);
...
function renderTrendChart(data) {
    ...
    const dataset = data.map(item => item.presence_rate);
...
// ------------------ NEW V2 HANDLERS: CCTV CAMERA CRUD & OVERRIDE ------------------

window.overrideAttendance = async function(sessionId, prn, status) {
    if (!confirm(`Are you sure you want to manually mark student ${prn} as ${status} for Session #${sessionId}?`)) {
        return;
    }
    
    try {
        const response = await fetch(`${API_URL}/api/attendance/override`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ session_id: sessionId, prn: prn, status: status })
        });
        ...
        if (response.ok) {
            ...
            // Re-render report view for current subject
            if (window.currentReportContext) {
                const classStudents = window.currentReportContext.classStudents;
                const subjectId = window.currentReportContext.subjectId;
                window.currentReportContext.subjectLogs = systemLogsCache.filter(log => log.subject_id === subjectId && classStudents.some(s => s.prn === log.prn));
                renderLogsReportTable();
            }
        } else {
        ...
        alert("Error overriding attendance: " + e.message);
    }
};

async function loadCamerasDirectory() {
    const listBody = document.getElementById("camera-list-body");
    if (!listBody) return;
    
    listBody.innerHTML = `<tr><td colspan="5" class="center"><i class="fa-solid fa-spinner fa-spin"></i> Loading cameras directory...</td></tr>`;
    ...
    try {
        ...
        if (cameras.length === 0) {
            listBody.innerHTML = `<tr><td colspan="5" class="center">No cameras registered in database.</td></tr>`;
            return;
        }
        ...
        cameras.forEach(cam => {
            const row = `
                <tr>
                    ...
                    <td><span class="status-pill present">${cam.direction}</span></td>
                    <td>
                        <div class="action-btn-group">
                            <button onclick="editCameraInline(${cam.camera_id}, '${cam.camera_name.replace(/'/g, "\\'")}', '${(cam.rtsp_url || '').replace(/'/g, "\\'")}', '${cam.direction}')" class="btn-edit-tbl">
                                <i class="fa-solid fa-pen-to-square"></i> Edit
                            </button>
                            <button onclick="confirmDeleteCamera(${cam.camera_id})" class="btn-delete-tbl">
                                <i class="fa-solid fa-trash-can"></i> Delete
                            </button>
                        </div>
                    </td>
        ...
        console.error(e);
        listBody.innerHTML = `<tr><td colspan="5" class="center text-danger">Failed to fetch CCTV cameras list.</td></tr>`;
    }
}
...
async function handleCameraFormSubmit(e) {
    ...
    const idInput = document.getElementById("camera-id");
    const nameInput = document.getElementById("camera-name-input");
    const rtspInput = document.getElementById("camera-rtsp-input");
    const dirSelect = document.getElementById("camera-direction-select");
    const statusMsg = document.getElementById("camera-status-msg");
    const submitBtn = document.getElementById("btn-camera-submit");
    
    const id = idInput.value;
    const name = nameInput.value.trim ? nameInput.value.trim() : nameInput.value;
    const rtsp = rtspInput.value.trim ? rtspInput.value.trim() : rtspInput;
    ...
    const url = id ? `${API_URL}/api/cameras/${id}/edit` : `${API_URL}/api/cameras`;
    const payload = { camera_name: name, rtsp_url: rtsp, direction: direction };
    ...
    try {
        ...
        alert(id ? "Camera updated successfully!" : "Camera added successfully!");
        resetCameraForm();
        loadCamerasDirectory();
    } catch(err) {
        ...
        submitBtn.innerHTML = `Save Camera`;
    }
}

window.editCameraInline = function(id, name, rtsp, direction) {
    document.getElementById("camera-id").value = id;
    document.getElementById("camera-name-input").value = name;
    document.getElementById("camera-rtsp-input").value = rtsp;
    document.getElementById("camera-direction-select").value = direction;
    
    document.getElementById("camera-form-title").innerHTML = `<i class="fa-solid fa-pen-to-square"></i> Edit Camera #${id}`;
    document.getElementById("btn-camera-cancel").classList.remove("hide");
    document.getElementById("camera-status-msg").classList.add("hide");
};

window.confirmDeleteCamera = async function(id) {
    if (!confirm(`Are you sure you want to permanently delete CCTV camera #${id}?`)) {
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            ...
            const currentId = document.getElementById("camera-id").value;
            if (currentId == id) {
                resetCameraForm();
            }
        } else {
...
function resetCameraForm() {
    ...
    document.getElementById("camera-rtsp-input").value = "";
    document.getElementById("camera-direction-select").value = "BOTH";
    
    document.getElementById("camera-form-title").innerHTML = `<i class="fa-solid fa-circle-plus"></i> Add New Camera`;
    document.getElementById("btn-camera-cancel").classList.add("hide");
    document.getElementById("camera-status-msg").classList.add("hide");
}
